...
import { EntityArrayResponseType, EmplacementService } from '../service/emplacement.service';
...
export class EmplacementComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEmplacement): number => this.emplacementService.getEmplacementIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.emplacements = this.refineData(dataFromBody);
  }

  protected refineData(data: IEmplacement[]): IEmplacement[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IEmplacement[] | null): IEmplacement[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.emplacementService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
