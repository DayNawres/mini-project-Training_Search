...
type RestOf<T extends IInscription | NewInscription> = Omit<T, 'dateValiditeDebut' | 'dateValiditeFin'> & {
...
export class InscriptionService {
  ...
  protected convertDateFromClient<T extends IInscription | NewInscription | PartialUpdateInscription>(inscription: T): RestOf<T> {
    return {
      ...inscription,
      dateValiditeDebut: inscription.dateValiditeDebut?.format(DATE_FORMAT) ?? null,
      dateValiditeFin: inscription.dateValiditeFin?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInscription: RestInscription): IInscription {
    return {
      ...restInscription,
      dateValiditeDebut: restInscription.dateValiditeDebut ? dayjs(restInscription.dateValiditeDebut) : undefined,
      dateValiditeFin: restInscription.dateValiditeFin ? dayjs(restInscription.dateValiditeFin) : undefined,
    };
  }
